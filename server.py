...
On first run, downloads the sentiment model (~500MB).
...
def get_sentiment_pipeline():
    ...
    if _sentiment_pipeline is None:
        logger.info("Loading sentiment model (first run may take a few minutes)...")
        try:
            ...
            logger.warning(f"Could not load sentiment model: {e}. Using rule-based fallback.")
            _sentiment_pipeline = None
...
# Matches both 12h and 24h formats, Android & iOS
...
STOPWORDS = {
    ...
    'and', 'or', 'but', 'not', 'be', 'was', 'are', 'were', 'been', 'being',
    'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could',
    'should', 'may', 'might', 'shall', 'can', 'i', 'you', 'he', 'she',
    'we', 'they', 'me', 'him', 'her', 'us', 'them', 'my', 'your', 'his',
    'this', 'that', 'these', 'those', 'so', 'if', 'as', 'by', 'with',
    'from', 'up', 'about', 'into', 'through', 'during', 'before', 'after',
    'yes', 'no', 'ok', 'okay', 'yeah', 'yea', 'lol', 'haha', 'hahaha',
    'its', "it's", "i'm", "don't", "doesn't", "didn't", "won't", "can't",
    'just', 'like', 'even', 'well', 'back', 'there', 'out', 'one', 'all',
    'what', 'which', 'who', 'when', 'where', 'how', 'why', 'get', 'got',
    'also', 'than', 'then', 'now', 'only', 'also', 'too', 'very', 'much',
}

def parse_whatsapp_chat(text: str) -> list[dict]:
    """Parse WhatsApp exported chat text into list of message dicts."""
    messages = []

    # Try Android format first, then iOS
...
def _parse_datetime(date_str: str, time_str: str) -> datetime:
    ...
    time_formats = ['%I:%M %p', '%I:%M%p', '%H:%M', '%H:%M:%S', '%I:%M:%S %p']
...
def analyse_chat(messages: list[dict]) -> dict:
    if not messages:
    ...
    # ── Per participant ───────────────────────────────────────────────────────
    participant_stats = []
    participant_sentiments_list = []

    # Collect texts per participant for sentiment batch
    sender_messages_text: dict[str, list[str]] = defaultdict(list)
    for m in messages:
        if not m['is_media'] and len(m['body']) > 10:
            sender_messages_text[m['sender']].append(m['body'])
    ...
    for sender in senders:
        ...
        p_most_active_hour = _fmt_hour(max(p_hourly, key=p_hourly.get)) if p_hourly else 'N/A'

        # Sentiment
        texts = sender_messages_text.get(sender, [])
        sentiment_score, mood = _compute_sentiment(pipe, texts)
    ...
    # ── Overall sentiment ────────────────────────────────────────────────────
    all_texts = [m['body'] for m in messages if not m['is_media'] and len(m['body']) > 10]
    overall_score, overall_mood = _compute_sentiment(pipe, all_texts, sample=200)
    ...
    total_p = max(len(participant_sentiments_list), 1)
    ...
    return {
        ...
        'top_emojis': [
            {'emoji': e, 'count': c}
            for e, c in Counter(all_emojis).most_common(20)
        ],
        'sentiment': {
            'overall_score': round(overall_score, 3),
            'overall_mood': overall_mood,
            'positive_percent': round(pos / total_p * 100, 1),
            'negative_percent': round(neg / total_p * 100, 1),
            'neutral_percent': round(neu / total_p * 100, 1),
...
def _compute_sentiment(pipe, texts: list[str], sample: int = 50) -> tuple[float, str]:
    """Return (score_in_-1_to_1, mood_label)."""
    if not texts:
        return 0.0, 'Neutral'
    ...
    if pipe is not None:
        try:
            ...
            logger.warning(f"Transformer sentiment failed: {e}. Using rule-based.")
...
def _compute_response_times(messages: list[dict], senders: list[str]) -> list[dict]:
    """Average response time per sender (in minutes)."""
    response_buckets: dict[str, list[float]] = defaultdict(list)
    ...
    for i in range(1, len(messages)):
        ...
        if prev['sender'] != curr['sender']:
            ...
            if 0 < delta < 1440:  # ignore gaps > 24h
                response_buckets[curr['sender']].append(delta)

    result = []
    for sender in senders:
        times = response_buckets.get(sender, [])
        if times:
            avg = sum(times) / len(times)
            result.append({'participant': sender, 'avg_minutes': round(avg, 1)})
...
def info():
    pipe = get_sentiment_pipeline()
    return {
        'sentiment_model': 'distilbert-base-uncased-finetuned-sst-2-english' if pipe else 'rule-based fallback',
...
async def analyze(file: UploadFile = File(...)):
    ...
    logger.info(f'Received file: {file.filename} ({len(text)} chars)')

    messages = parse_whatsapp_chat(text)
    if not messages:
        return {
            'success': False,
            'error': 'Could not parse any messages from this file. '
                     'Make sure it is a WhatsApp exported chat (.txt) file.',
        }
...
if __name__ == '__main__':
    ...
    print('The sentiment model will download on first use.')
